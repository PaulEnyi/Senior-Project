...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const endpoint = mode === 'signup' ? '/api/auth-db/signup' : '/api/auth-db/login';
      const response = await fetch(`http://localhost:8000${endpoint}`, {
        ...
        body: JSON.stringify({
          ...
          student_id: mode === 'signup' ? formData.studentId : undefined,
          major: mode === 'signup' ? formData.major : undefined,
          concentration: mode === 'signup' ? formData.concentration : undefined,
          classification: mode === 'signup' ? formData.classification : undefined,
          expected_graduation: mode === 'signup' ? formData.expectedGraduation : undefined
        })
      });
      ...
      localStorage.setItem('role', userData.role || (isAdmin ? 'admin' : 'user'));
      localStorage.setItem('user', JSON.stringify(userData));
      console.log('  ✓ User data stored');
      
      // Load chat threads (will be displayed in Chat History)
      if (userData.chat_threads && userData.chat_threads.length > 0) {
        localStorage.setItem('chat_threads', JSON.stringify(userData.chat_threads));
        console.log(`  ✓ Loaded ${userData.chat_threads.length} previous chat threads`);
      } else {
        console.log('  ℹ️ No previous chats found');
      }
      
      // Store DegreeWorks data if available
      if (userData.degree_works_data) {
        localStorage.setItem('degree_works_data', JSON.stringify(userData.degree_works_data));
        console.log('  ✓ Loaded DegreeWorks analysis');
      }
      
      // Store profile data
      if (userData.major || userData.classification || userData.student_id) {
        const profile = {
          student_id: userData.student_id,
          major: userData.major,
          concentration: userData.concentration,
          classification: userData.classification,
          expected_graduation: userData.expected_graduation,
          gpa: userData.gpa
        };
      ...
      console.log('ℹ️  Starting with fresh chat interface');
      ...
      navigate(redirectTo, { state: { newChat: true } }); // Force fresh chat on login
    } catch (err) {
      ...
      <div className="login-container">
        ...
        <section className={`credentials-slide ${showCredentialsPanel ? 'open' : ''}`}>
          ...
          <form className="credentials-form" onSubmit={handleSubmit}>
            ...
            {mode === 'signup' && (
              <>
                <div className="form-divider" style={{margin: '16px 0', borderTop: '1px solid #e5e7eb', paddingTop: '12px'}}>
                  <p style={{fontSize: '13px', color: '#6b7280', marginBottom: '12px'}}>Optional: Academic Information (can be added later)</p>
                </div>
                
                <div className="form-row">
                  <label>Student ID (Optional)</label>
                  <div className="input-wrap"><FiUser className="in-ico" /><input type="text" name="studentId" value={formData.studentId} onChange={handleInputChange} placeholder="e.g., 20240001" /></div>
                </div>
                
                <div className="form-row">
                  <label>Major (Optional)</label>
                  <div className="input-wrap"><FiUser className="in-ico" /><input type="text" name="major" value={formData.major} onChange={handleInputChange} placeholder="e.g., Computer Science" /></div>
                </div>
                
                <div className="form-row">
                  <label>Concentration (Optional)</label>
                  <div className="input-wrap"><FiUser className="in-ico" /><input type="text" name="concentration" value={formData.concentration} onChange={handleInputChange} placeholder="e.g., Software Engineering" /></div>
                </div>
                
                <div className="form-row">
                  <label>Classification (Optional)</label>
                  <div className="input-wrap">
                    <FiUser className="in-ico" />
                    <select name="classification" value={formData.classification} onChange={handleInputChange} style={{width: '100%', padding: '10px 12px 10px 36px', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '14px'}}>
                ...
                <div className="form-row">
                  <label>Expected Graduation (Optional)</label>
                  <div className="input-wrap"><FiUser className="in-ico" /><input type="text" name="expectedGraduation" value={formData.expectedGraduation} onChange={handleInputChange} placeholder="e.g., Spring 2025" /></div>
                </div>
              </>
            )}
