...
class OpenAIService:
    ...
    def _get_system_prompt(self) -> str:
        ...
        return """You are Morgan AI, the CS Assistant for Morgan State University. Answer questions about:
- CS courses, prerequisites, degree requirements
- Student registration and academic planning  
- Faculty, office hours, internships
- Use knowledge base and web search when provided
- Cite sources with URLs
- For student transcripts, reference their Degree Works data if provided
- Be professional, accurate, and helpful"""
    ...
    async def generate_chat_response(
        ...
        try:
                ...
                # Get conversation history - LIMIT to 5 messages to reduce token usage
                history = await self.thread_manager.get_messages(session_id, limit=3)
    ...
    async def _get_rag_context(self, query: str, top_k: int = 3) -> str:
        ...
        Strictly limited to max 1500 chars to prevent token overload."""
        try:
            ...
            # Format context with length cap
            context_parts = []
            context_length = 0
            max_context_length = 1500  # Hard cap on context length
            ...
            for i, match in enumerate(results):
                ...
                if metadata and context_length < max_context_length:
                    ...
                    if score and score > 0.65:
                        # Truncate text if necessary to stay under max_context_length
                        remaining_space = max_context_length - context_length - 100
                        if len(text) > remaining_space and remaining_space > 200:
                            text = text[:remaining_space] + "..."
                        
                        formatted_part = f"[Source: {source}]\n{text}"
                        context_parts.append(formatted_part)
                        context_length += len(formatted_part)
            ...
            if context_parts:
                ...
                if any(k in q_lower for k in forced_keywords):
                    try:
                        ...
                        web_results = await self.web_search_service.deep_search_morgan(query, max_results=2)
                        if web_results.get("success") and web_results.get("content"):
                            # Limit web search addition to 800 chars
                            web_content = web_results['content'][:800]
                            deep_context = f"[DEEP SEARCH AUGMENTATION]\n{web_content}"
                            if context_length + len(deep_context) <= max_context_length:
            ...
            if web_results.get("success") and web_results.get("content"):
                # Limit web context to max_context_length
                web_content = web_results['content'][:max_context_length]
                web_context = f"[DEEP SEARCH - Morgan State Official Sources]\n{web_content}"
                
                logger.info(f"[MorganAI RAG] Deep web search successful (limited to {len(web_context)} chars)!")
