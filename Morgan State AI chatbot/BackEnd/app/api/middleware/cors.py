...
def setup_rate_limiting_headers(app: FastAPI) -> None:
    ...
    async def add_rate_limit_headers(request, call_next):
        ...
        # Add rate limiting headers (values from actual rate limiter)
...
def configure_middleware(app: FastAPI) -> None:
    ...
    setup_rate_limiting_headers(app)  # 1. Rate limiting headers
    setup_request_id_middleware(app)  # 2. Request ID tracking
    setup_security_headers(app)       # 3. Security headers
    setup_trusted_hosts(app)          # 4. Trusted hosts validation
    setup_cors(app)                   # 5. CORS (handles preflight)
