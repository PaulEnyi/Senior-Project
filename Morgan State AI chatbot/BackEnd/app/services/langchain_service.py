    ...
    def _json_to_text(self, data: Any, filename: str) -> str:
        ...
        if isinstance(data, dict):
                ...
                if isinstance(value, (str, int, float)):
                    ...
                    for item in value:
                        if isinstance(item, str):
                            text_parts.append(f"  • {item}")  # FIXED: proper bullet
                        elif isinstance(item, dict):
                            text_parts.append(f"  • {self._dict_to_text(item)}")  # FIXED
            ...
            return f"Information from {filename}:\n" + "\n".join([
                f"• {item}" if isinstance(item, str) else f"• {self._dict_to_text(item)}"  # FIXED
                for item in data
            ])
