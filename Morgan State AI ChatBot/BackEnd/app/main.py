...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        try:
            ...
            services_initialized["openai"] = True
        ...
        try:
            ...
            services_initialized["pinecone"] = True
            logger.info("✓ Pinecone service initialized")
        except Exception as e:
            logger.error(f"✗ Pinecone service initialization failed: {str(e)}")
            if not settings.DEBUG:
                raise  # Fail startup in production
        
        # Log initialization summary
        if all(services_initialized.values()):
            logger.info("✓ All services initialized successfully")
        else:
            failed_services = [k for k, v in services_initialized.items() if not v]
            logger.warning(f"⚠ Running with degraded services. Failed: {', '.join(failed_services)}")
        
        logger.info(f"Morgan AI Chatbot started successfully on {settings.HOST}:{settings.PORT}")
        
    except Exception as e:
        logger.error(f" Failed to initialize services: {str(e)}")
        if not settings.DEBUG:
            raise e  # Prevent startup in production if services fail
...
# Get client IP (supports proxy headers)
    ...
    # Skip rate limiting for health checks and admin endpoints in debug mode
    skip_rate_limit = (
        request.url.path in ["/health", "/health/detailed", "/"] or
        (settings.DEBUG and request.url.path.startswith("/api/admin"))
    )
...
async def detailed_health_check():
    ...
    try:
        ...
        try:
            pinecone_service = PineconeService()
            await pinecone_service.test_connection()
            services_status["pinecone"] = "healthy"
        except Exception as e:
            services_status["pinecone"] = f"unhealthy: {str(e)}"
            health_status["status"] = "degraded"
